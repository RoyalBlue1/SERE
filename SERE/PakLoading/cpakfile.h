...
struct PakSegmentHdr_t
{
    ...
    inline const bool IsHeaderSegment() const
    {
        return GetType() == 0;
    }
};
...
protected:
    ...
    inline std::shared_ptr<char[]> GetDecompressedBuffer() { return m_Buf; };

    inline void SetPatchBytesToSkip(size_t numBytes) { p.numBytesToSkip = numBytes; };
    inline void SetPatchSourceData(void* pointer) { p.patchOriginalData = reinterpret_cast<char*>(pointer); };

    inline void SetPatchDestination(void* pointer, size_t destinationSize)
    {
...
private:
    ...
    const bool ParseFromFile(const std::filesystem::path& filePath, std::shared_ptr<char[]>& buf);
...
public:
    ...
    std::unique_ptr<char[]> getStarPakData(const uint64_t offset, const uint64_t size, const bool opt) const
    {
        ...
        std::ifstream file{pakEntry->filePath,std::ios::binary};
        if(!file.good())
            return nullptr;
